# -*- coding: utf-8 -*-
"""
Admin Agent Exaone - Clinical Note 생성 에이전트 (Exaone 3.5 7.8B)
Ollama를 통해 로컬에서 실행되는 Exaone 3.5 7.8B 모델을 사용합니다.
한글 특화 모델로 한국어 Clinical Note 생성에 최적화되어 있습니다.
"""
...
from config.config import LLM_CONFIG, OLLAMA_GUIDE
from data_extractor import PatientDataExtractor


class AdminAgentExaone:
    """
    Admin Agent Exaone 클래스

    로컬 Exaone 3.5 7.8B 모델을 사용하여 Clinical Note를 생성합니다.
    한글과 영어 버전을 각각 생성합니다.
    """

    def __init__(self):
        """
        Admin Agent Exaone 초기화
        - Ollama 서버 연결
        ...
        self.korean_agent = AssistantAgent(
            name="Korean_Clinical_Note_Writer_Exaone",
...
의학 용어는 정확하게 사용하고, 한글과 영문을 적절히 병기하세요.""",
            llm_config=self.llm_config
        )

        # AutoGen AssistantAgent 생성 (영어용)
        self.english_agent = AssistantAgent(
            name="English_Clinical_Note_Writer_Exaone",
...
When you receive patient data, write a natural and professional Clinical Note in English following the above format.""",
        ...
        print("[OK] Admin Agent Exaone 초기화 완료")
        print(f"  - Model: {LLM_CONFIG['model']} (한글 특화)")
...
위 데이터를 종합하여 자연스러운 Clinical Note를 작성해주세요."""
    ...
    def generate_clinical_note(self, patient_data, language="korean"):
        ...
        print(f"\n{'='*60}")
        print(f"Clinical Note 생성 중 ({language.upper()}) - Exaone 3.5 7.8B...")
        print(f"{'='*60}")
    ...
    def save_clinical_note(self, clinical_note, patient_id, language="korean"):
        ...
        filename = f"{patient_id}_{language}_exaone_{timestamp}.txt"
...
def main():
    ...
    """
    print("\n" + "="*60)
    print("Admin Agent Exaone - Clinical Note 생성 시스템")
    print("Exaone 3.5 7.8B (한글 특화 모델)")
    print("="*60 + "\n")

    # Ollama 가이드 출력 (자동 실행 모드)
    print("\n[INFO] Ollama 서버 연결을 시도합니다...")
    print("[INFO] Ollama가 실행 중인지 확인하세요: http://localhost:11434")
    ...
    # 2. Admin Agent Exaone 초기화
    print("\n2단계: Admin Agent Exaone 초기화")
    print("-" * 60)
    try:
        admin_agent = AdminAgentExaone()
    except Exception as e:
