# -*- coding: utf-8 -*-
"""
Admin Agent Local - Clinical Note 생성 에이전트 (Llama 3.1 8B)
Ollama를 통해 로컬에서 실행되는 Llama 3.1 8B 모델을 사용합니다.
"""
...
class AdminAgentLocal:
    """
    Admin Agent Local 클래스

    로컬 Llama 3.1 8B 모델을 사용하여 Clinical Note를 생성합니다.
    한글과 영어 버전을 각각 생성합니다.
    """

    def __init__(self):
        """
        Admin Agent Local 초기화
        - Ollama 서버 연결
        ...
        self.korean_agent = AssistantAgent(
            name="Korean_Clinical_Note_Writer_Local",
...
환자 데이터를 받으면 위 형식으로 자연스럽고 전문적인 한글 Clinical Note를 작성해주세요.""",
        ...
        self.english_agent = AssistantAgent(
            name="English_Clinical_Note_Writer_Local",
...
When you receive patient data, write a natural and professional Clinical Note in English following the above format.""",
        ...
        print("[OK] Admin Agent Local 초기화 완료")
...
위 데이터를 종합하여 자연스러운 Clinical Note를 작성해주세요."""
    ...
    def generate_clinical_note(self, patient_data, language="korean"):
        ...
        print(f"\n{'='*60}")
        print(f"Clinical Note 생성 중 ({language.upper()}) - Llama 3.1 8B...")
        print(f"{'='*60}")
    ...
    def save_clinical_note(self, clinical_note, patient_id, language="korean"):
        ...
        filename = f"{patient_id}_{language}_llama_{timestamp}.txt"
...
def main():
    ...
    """
    print("\n" + "="*60)
    print("Admin Agent Local - Clinical Note 생성 시스템")
    print("Llama 3.1 8B (Ollama)")
    print("="*60 + "\n")
    ...
    # 2. Admin Agent Local 초기화
    print("\n2단계: Admin Agent Local 초기화")
    print("-" * 60)
    try:
        admin_agent = AdminAgentLocal()
    except Exception as e:
        print(f"\n[ERROR] Ollama 서버에 연결할 수 없습니다.")
        print(f"오류: {str(e)}")
        print("\nOllama가 실행 중인지 확인하세요:")
        print("  1. 명령 프롬프트에서 'ollama serve' 실행")
        print("  2. 또는 'ollama run llama3.1:8b' 실행")
        sys.exit(1)
