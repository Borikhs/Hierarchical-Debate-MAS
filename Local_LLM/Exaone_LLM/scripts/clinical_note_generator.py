...
**STRICTLY FORBIDDEN**: Do NOT add any explanations, summaries, or comments at the end like "This note...", "This Clinical Note...", "This Full Cycle Clinical Note comprehensively...", etc. End with the [Disposition] section only."""
    ...
    def process_all_patients(self, test_mode=True, checkpoint_interval=1000):
        """
        모든 환자 또는 테스트 환자 1명을 처리합니다.
        Resume 기능: 이미 처리된 환자는 건너뜁니다.

        Args:
            test_mode (bool): True면 1명만, False면 전체 환자
            checkpoint_interval (int): 중간 저장 간격 (기본 1000명)
        """
        print("\n" + "="*70)
        ...
        if os.path.exists(output_base):
            processed_patients = set(os.listdir(output_base))

        # 처리할 환자 필터링
        patients_to_process = [pid for pid in patient_ids if str(pid) not in processed_patients]
        skipped_count = len(patient_ids) - len(patients_to_process)

        if skipped_count > 0:
            print(f"\n[RESUME] {skipped_count}명 이미 처리됨 → 건너뜀")
            print(f"[RESUME] {len(patients_to_process)}명 처리 예정")

        # 진행 상황 로그 파일
        log_file = os.path.join(output_base, "progress_log.txt")
        ...
        for idx, patient_id in enumerate(patients_to_process, 1):
            try:
                ...
                if success_count % checkpoint_interval == 0:
                    checkpoint_msg = f"[CHECKPOINT] {success_count}명 처리 완료 (총 {len(patients_to_process)}명 중)"
                    print(f"\n{'='*70}")
                    ...
                    with open(log_file, 'a', encoding='utf-8') as f:
                        f.write(f"{datetime.now().isoformat()} - {checkpoint_msg}\n")

                # 진행률 표시 (100명마다)
                if success_count % 100 == 0:
                    progress = (success_count / len(patients_to_process)) * 100
                    print(f"\n[진행률] {success_count}/{len(patients_to_process)} ({progress:.1f}%)")

            except Exception as e:
                error_count += 1
                error_msg = f"환자 {patient_id} 처리 중 오류: {e}"
                print(f"\n[ERROR] {error_msg}")
        ...
        print(f"  - 건너뜀 (이미 처리): {skipped_count}명")
        print(f"  - 저장 위치: {output_base}")
        print("="*70)

        # 최종 로그 기록
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(f"{datetime.now().isoformat()} - COMPLETED: 성공 {success_count}, 실패 {error_count}, 건너뜀 {skipped_count}\n")
...
def main():
    ...
    # 데이터 경로 설정 (GPU 서버용 Linux 경로)
    data_path = "/data/multi-agent_snuh/Exaone/data/SNUH_Data"
