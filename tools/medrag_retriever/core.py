...
class CustomizeSentenceTransformer(SentenceTransformer): # change the default pooling "MEAN" to "CLS"
    ...
    def _load_auto_model(self, model_name_or_path, *args, **kwargs):
        ...
        """
        print(f"[MedRAG] Loading transformer model '{model_name_or_path}' with CLS pooling...")
...
class Retriever: 
    ...
    def idx2txt(self, indices): # return List of Dict of str
        ...
        for i in indices:
            try:
                fpath = os.path.join(self.chunk_dir, i["source"] + ".jsonl")
                with open(fpath, 'r', encoding='utf-8') as f:
                    lines = f.read().strip().split('\n')
                    if i["index"] < len(lines):
                        results.append(json.loads(lines[i["index"]]))
                    else:
                        print(f"Warning: Index {i['index']} out of range for {i['source']}.jsonl (has {len(lines)} lines)")
                        # Return empty document on error
                        results.append({"id": f"{i['source']}_{i['index']}", "title": "Error", "content": "Document not found"})
            except Exception as e:
                print(f"Error reading {i['source']}.jsonl at index {i['index']}: {e}")
                results.append({"id": f"{i['source']}_{i['index']}", "title": "Error", "content": "Error reading document"})
        return results
