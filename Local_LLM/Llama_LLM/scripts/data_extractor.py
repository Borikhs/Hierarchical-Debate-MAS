...
from config.config import INITIAL_CONSULTATION_FILES, EXCLUDED_FILES
...
class PatientDataExtractor:
    ...
    def load_basic_info(self):
        """
        기본정보 파일을 로딩하고 처음 2명의 환자 ID를 추출합니다.

        Returns:
            list: 처음 2명의 환자 ID 리스트
        """
        file_path = os.path.join(self.data_path, INITIAL_CONSULTATION_FILES["1_기본정보"])
        df = pd.read_excel(file_path)

        # 처음 2명의 환자 데이터
        first_two_patients = df.head(2)

        # 환자 ID 추출 (첫 번째 컬럼)
        patient_ids = first_two_patients.iloc[:, 0].tolist()

        # 기본정보 저장
        for idx, patient_id in enumerate(patient_ids):
            self.patient_data[patient_id] = {
                "기본정보": first_two_patients.iloc[idx].to_dict()
            }
    ...
    def extract_all(self):
        ...
        # 1. 기본정보에서 처음 2명의 환자 ID 추출
        patient_ids = self.load_basic_info()
